    ...
    protected void tick() {
        ...
        for (Player player : playerListU.getPlayingPlayers()) {
            ...
            if (healthPercentage > LOW_HEALTH_THRESHOLD && healthPercentage < MID_HEALTH_THRESHOLD) {
                soundU.playSoundAtLocation(player.getLocation(), Sound.BLOCK_NOTE_BLOCK_BASEDRUM, 0.3F, 0.2F);
                new BukkitRunnable() {
                    @Override
                    public void run() {
                        soundU.playSoundAtLocation(player.getLocation(), Sound.BLOCK_NOTE_BLOCK_BASEDRUM, 0.3F, 0.2F);
            ...
            if (healthPercentage <= LOW_HEALTH_THRESHOLD) {
                soundU.playSoundAtLocation(player.getLocation(), Sound.BLOCK_NOTE_BLOCK_BASEDRUM, 0.4F, 0.4F);
                new BukkitRunnable() {
                    @Override
                    public void run() {
                        soundU.playSoundAtLocation(player.getLocation(), Sound.BLOCK_NOTE_BLOCK_BASEDRUM, 0.4F, 0.3F);
                    }
